...
public class AI : MonoBehaviour {
	...
	private enum State {
		Idle,				//do nothing
		Init,				//maked sure that everything we need is here
		Setup,				//assign the values to things we need
...
/*		SphereCollider sc = GetComponent<SphereCollider> ();
		...
		if (sc == null) {
			Debug.LogError("There is no sphere collider on this mob");
		}
		else {
			sc.isTrigger = true;
		}
		...
		if (cc == null) {
			Debug.LogError("There is no character controller on this mob");
		}
		else {
			sc.center = cc.center;
